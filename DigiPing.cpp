...
// ICMP Structure
struct ICMPPacket {
    // TODO: Identify the values and their types needed here
    // (see the ICMP packet documentation)
...
static void fill_icmp_packet(ICMPPacket& packet, uint16_t id, uint16_t sequence)
{
    ...
    uint16_t checksum = compute_checksum(&packet, sizeof(ICMPPacket));
}
...
int main(int argc, char** argv)
{
    ...
    while (g_running)
    {
        ...
        // TODO: otherwise, determine the type of the reply and print the appropriate response
        // Is this an ICMP_TYPE_ECHOREPLY, and does it have the id we expect? (should be pid)
        else if (1 == 0 /* type==ICMP_TYPE_ECHOREPLY && id==pid*/)
        {
